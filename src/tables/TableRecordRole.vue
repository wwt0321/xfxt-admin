<template>
  <div>
    <div class="top-filter">
      ...
      <q-input v-model="filters.name" outlined dense class="top-filter-input" placeholder="请输入角色名称"></q-input>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { name: 'way', label: '补贴途径', field: 'way', align: 'center', format: (v) => '角色补贴' },
        {
          name: 'state',
          label: '补贴当前状态',
          field: 'state',
  ...
  methods: {
    async refresh() {
      this.selected = [];
      let url = `/record/role?limit=${this.pagination.rowsPerPage}&page=${this.pagination.page}`;
      if (this.filters) {
    ...
    async refund(row) {
      if (row.state == 1) {
        let result = await this.confirm(`确认对“${row.roleName}”进行退补?`);
        if (result) {
          this.loading++;
    ...
    async regrant(row) {
      let result = await this.confirm(`确认对“${row.roleName}”进行重新补贴?`);
      if (result) {
        this.loading++;
        let params = new FormData();
        params.append('roleId', row.roleId);
        params.append('roleName', row.roleName);
